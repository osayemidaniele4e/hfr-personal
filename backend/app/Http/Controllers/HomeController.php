...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        $geo_percent = Cache::remember('percent_facilities_geo', 30, function () {
            return DB::select("SELECT state_id as name, cast(SUM(case when latitude <> '' then 1 else 0 end)/count(id)*100 as unsigned) as y
                    FROM hospital_details group by state_id order by y desc");
        });
    ...
    public function getFacilitesByLGA(Request $request)
    {
        ...
        $by_level = DB::select("SELECT facility_level_id as name,COUNT(id) AS y FROM hospital_details 
                WHERE state_id=" . $state_id . " GROUP BY facility_level_id order by facility_level_id");

        //by ownership
        $by_ownership =  DB::select("SELECT ownership_id as name,COUNT(id) AS y FROM hospital_details 
