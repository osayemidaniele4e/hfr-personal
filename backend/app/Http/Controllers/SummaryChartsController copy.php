...
class SummaryChartsController extends Controller
{
    ...
    public function index(){
        ...
        return view('public.statistic_charts', compact('facilities_ownership_state','facilities_level_state','levels_by_state',
    ...
    public function filter(Request $request){
        ...
        if ($facility_type_id==1){   
            ...
            if($state_id==0){
                $facilities_level_state = Cache::remember('facilities_level_state', 30, function () {
                    return DB::select("SELECT facility_level as name,COUNT(id) AS y FROM hospital_details 
                                GROUP BY facility_level order by facility_level");
                });
    
                //by ownership
                $facilities_ownership_state = Cache::remember('facilities_ownership_state', 30, function () {
                    return DB::select("SELECT ownership as name,COUNT(id) AS y FROM hospital_details 
                                GROUP BY ownership order by ownership");
                });
                ...
                $facilities_level_state = DB::select("SELECT facility_level as name,COUNT(id) AS y FROM hospital_details 
                                WHERE state_id=". $state_id ." GROUP BY facility_level order by facility_level");
    
    
                //by ownership
                $facilities_ownership_state =  DB::select("SELECT ownership as name,COUNT(id) AS y FROM hospital_details 
            ...
            return view('public.statistic_charts', compact('facilities_level_state','facilities_ownership_state','levels_by_state',
            'lst_facility_types','lst_states','facility_type_id','state_id','filtered'));
        }
        //pharmacies
        if ($facility_type_id==2){
            if($state_id==0){
                //by ownership
                $facilities_ownership_state = DB::select("SELECT ownership name,COUNT(id) AS y FROM pharmacy_details 
                                GROUP BY ownership order by ownership"); 
            }
            else{
     
                $facilities_ownership_state =  DB::select("SELECT ownership as name,COUNT(id) AS y FROM pharmacy_details 
                                WHERE state_id=". $state_id ."  GROUP BY ownership order by ownership");
            
            }
       
            return view('public.statistic_charts2', compact('facilities_ownership_state',
            'lst_facility_types','lst_states','facility_type_id','state_id','filtered'));
        }
        //labs
        if ($facility_type_id==3){ 
               ...
               if($state_id==0){
                    $facilities_level_state = Cache::remember('lab_level_state', 30, function () {
                        return DB::select("SELECT facility_level as name,COUNT(id) AS y FROM laboratory_details 
                                    GROUP BY facility_level order by facility_level");
                    });
                  
                    //by ownership
                    $facilities_ownership_state = Cache::remember('lab_ownership_state', 30, function () {
                        return DB::select("SELECT ownership name,COUNT(id) AS y FROM laboratory_details 
                                    GROUP BY ownership order by ownership");
                    });
                    ...
                    $facilities_level_state = DB::select("SELECT facility_level as name,COUNT(id) AS y FROM laboratory_details 
                                    WHERE state_id=". $state_id ." GROUP BY facility_level order by facility_level");
                
                    //by ownership
                    $facilities_ownership_state =  DB::select("SELECT ownership as name,COUNT(id) AS y FROM laboratory_details 
                                    WHERE state_id=". $state_id ."  GROUP BY ownership order by ownership");
        ...
        if ($facility_type_id==4){
            if($state_id==0){
                ...
                $facilities_ownership_state = DB::select("SELECT ownership name,COUNT(id) AS y FROM imaging_details 
                                GROUP BY ownership order by ownership"); 
            }
            else{
     
                $facilities_ownership_state =  DB::select("SELECT ownership as name,COUNT(id) AS y FROM imaging_details 
                                WHERE state_id=". $state_id ."  GROUP BY ownership order by ownership");
