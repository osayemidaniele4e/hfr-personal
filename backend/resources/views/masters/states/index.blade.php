              ...
              <tr>
                ...
                <td>
                    @if(auth()->user()->hasPermissionTo(49))
                      <a href="#">
                        <button class="btn btn-warning btn-sm" data-id="{{$st->id}}" data-name="{{$st->name}}" data-code="{{$st->short_code}}" 
                          data-num_code="{{$st->num_code}}" type="button" data-toggle="modal" data-target="#editModal">Edit</button>
                      </a>
                    @endif
