                            ...
                            <td>
                                @if (auth()->user()->hasPermissionTo(9))
                                    <a href="javascript:void(0);">
                                        <button class="btn btn-success btn-sm view-pharmacy"
                                            data-id="{{ $ph->id }}">View</button>
                                ...
                                @if (auth()->user()->hasPermissionTo(12))
                                    <a href="#">
                                        <button class="btn btn-danger btn-sm" data-id="{{ $ph->id }}"
                                            data-name="{{ $ph->facility_name }}" type="button" data-toggle="modal"
...
@push('bk_script')
    ...
    <script>
        ...
        $(document).on('click', '.view-pharmacy', function() {
            const id = $(this).data('id');
            $.ajax({
                url: `/admin/pharmacies/show/${id}`, // Adjust route as needed
                type: 'GET',
                success: function(response) {
                    $("#dynamicModalContainer").html(response);
                    $("#pharmacyModal" + id).modal("show");
                    // $('#pharmacy-details').html(data.html);
                },
                error: function() {
                    $('#pharmacy-details').html('<p class="text-danger">Failed to load details.</p>');
                }
            });
        });
    </script>
