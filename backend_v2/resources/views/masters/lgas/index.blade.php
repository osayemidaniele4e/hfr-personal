        ...
        <div class="box-body">
            <form class="form-horizontal" action="{{ route('lga.search') }}" method="get">
                ...
                <div class="form-group">
                    ...
                    <div class="col-sm-4">
                        <input class="form-control input-sm"type="text" name="name" id="name" class="form-control"
                        ...
                        <tr>
                            ...
                            <td>
                                @if (auth()->user()->hasPermissionTo(53))
                                    <a href="#">
                                        <button class="btn btn-warning btn-sm" data-id="{{ $lga->id }}"
                                            data-name="{{ $lga->name }}" data-lga_code="{{ $lga->lga_code }}"
                                            data-state_id="{{ $lga->state_id }}" type="button" data-toggle="modal"
                                            data-target="#editModal">Edit</button>
                                    </a>
                                @endif
                                @if (auth()->user()->hasPermissionTo(54))
                                    <a href="#">
                                        <button class="btn btn-danger btn-sm" data-id="{{ $lga->id }}"
                                            data-name="{{ $lga->name }}" type="button" data-toggle="modal"
