                ...
                <div class="form-group">
                    ...
                    <div class="col-md-5">
                        <select class="form-control select2" class="form-control" name="status" data-width="100%">
                            <option value="1" {{ '1' == old('status') ? 'selected' : '' }}>My Pending Validations
                            </option>
                            <option value="2" {{ '2' == old('status') ? 'selected' : '' }}>My Accepted Validations
                                (Pending Publication)</option>
                            <option value="3" {{ '3' == old('status') ? 'selected' : '' }}>My Rejected Validations
                            </option>
                    ...
                    <tbody>
                        @foreach ($pending as $p)
                            <tr>
                                ...
                                <td>
                                    <Strong></Strong>{{ $p->requested_by_lastname }} {{ $p->requested_by_firstname }}<br>
                                ...
                                <td>
                                    @if (in_array($p->status_id, [2, 7, 9, 14, 16, 21]))
                                        @if ($p->action === 'CREATE FACILITY')
                                            <a href="#">
                                                <button class="btn btn-success btn-sm" type="button" data-toggle="modal"
                                                    ...
                                                    data-ownership_type="{{ $p->ownership_type }}"
                                                    data-facility_level="{{ $p->facility_level }}"
                                                    data-facility_level_option="{{ $p->facility_level_option }}"
                                                    data-physical_location="{{ $p->physical_location }}"
                                                    data-alternate_number="{{ $p->alternate_number }}"
                                                    ...
                                                    data-operation_status="{{ $p->operation_status }}"
                                                    data-registration_status="{{ $p->registration_status }}"
                                                    data-license_status="{{ $p->license_status }}"
                                        ...
                                        @elseif ($p->action === 'UPDATE FACILITY')
                                            <a
                                                href="{{ route('view.updated_records', ['id' => $p->id, 'stage' => '2']) }}">
    ...
    <div class="modal fade" id="recall" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
        <div class="modal-dialog modal-sm" role="document">
            <div class="modal-content">
                ...
                <form action="{{ route('validate.recall') }}" method="POST">
                    @csrf

                    <div class="modal-body">
                        <p class="text-center">
                            Are you sure you want to recall validation for this facility?
                        </p>
