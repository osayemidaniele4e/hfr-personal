...
@section('content')
    ...
    <div class="box box-primary">
        <div class="box-header with-border">
            <h3 class="box-title">Updated values for <font color="MediumSeaGreen"><Strong>{{ $name }}</Strong></font>
            </h3>
                                ...
                                <tr>
                                    ...
                                    @if (!($audit['old'] == '' and $audit['new'] == ''))
                                        ...
                                        @elseif($attr === 'ownership_type_id')
                                            <td>{{ $old_values->ownership_type == '' ? 'null' : $old_values->ownership_type }}
                                            </td>
                                            <td>to</td>
                                            <td>{{ $new_values->ownership_type }}</td>
                                        @elseif($attr === 'facility_level_id')
                                            <td>{{ $old_values->facility_level == '' ? 'null' : $old_values->facility_level }}
                                            </td>
                                            <td>to</td>
                                            <td>{{ $new_values->facility_level }}</td>
                                        @elseif($attr === 'facility_level_option_id')
                                            <td>{{ $old_values->facility_level_option == '' ? 'null' : $old_values->facility_level_option }}
                                            </td>
                                            <td>to</td>
                                            <td>{{ $new_values->facility_level_option }}</td>
                                        @elseif($attr === 'operational_status_id')
                                            <td>{{ $old_values->operation_status == '' ? 'null' : $old_values->operation_status }}
                                            </td>
                                            <td>to</td>
                                            <td>{{ $new_values->operation_status }}</td>
                                        @elseif($attr === 'registration_status_id')
                                            <td>{{ $old_values->registration_status == '' ? 'null' : $old_values->registration_status }}
                                            </td>
                                            <td>to</td>
                                            <td>{{ $new_values->registration_status == '' ? 'null' : $new_values->registration_status }}
                                            </td>
                                        @elseif($attr === 'license_status_id')
                                            <td>{{ $old_values->license_status == '' ? 'null' : $old_values->license_status }}
                                            </td>
                                            <td>to</td>
